...
def app():
    ...
    st.sidebar.subheader("Introduire de l'inertie dans le score :")
    ...
    scores.append(r2test2*100)
    diff = round((scores[-1]-scores[-2]),1)
    if diff > 80:
        diff = 0

    st.metric("Score sur l'ensemble de test (%)*", round(r2test2*100,1), delta=diff, delta_color="normal")
